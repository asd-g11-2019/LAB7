...
Error tree::addElem(const Label labelOfNodeInTree, const Label labelOfNodeToAdd, Tree& t)
{
  ...
  newNode->label = labelOfNodeToAdd;
  newNode->firstChild = emptyTree;
  ...
  Tree exists = getNode(labelOfNodeToAdd, t);

  if (exists == emptyTree) {
    Tree subTree = getNode(labelOfNodeInTree, t);

    if (subTree != emptyTree) {
      if (subTree->firstChild == emptyTree) {
        subTree->firstChild = newNode;
        return OK;
      }

      Tree auxT = subTree->firstChild;

      while (auxT->nextSibling != emptyTree)
        auxT = auxT->nextSibling;

      auxT->nextSibling = newNode;
...
Label tree::father(const Label l, const Tree& t)
{
  ...
  Label rl = tree::father(l, t->firstChild);
  if (rl != emptyLabel)
    return rl;
  
  return tree::father(l, t->nextSibling);
}
...
void printRicursive (const Tree& t, int level = 0) {
  if (isEmpty(t))
    return;
  ...
  cout << spacing << t->label << endl;
  printRicursive(t->firstChild, level + 1);
  printRicursive(t->nextSibling, level);
}
